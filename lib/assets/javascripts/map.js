...
function open_socket(url){
	...
	socket.onopen = function(event){
		...
		var request = JSON.stringify({engin: "gamemanage", action: "enter_game"});
		socket.send(request);
	};
	...
	socket.onmessage = function(event) { 
		...
		if (res['action'] == "get ready"){
			...
			var request = JSON.stringify({engin: "gamemanage", action: "get_players"});
			socket.send(request);
		}
		else if (res['action'] == "get players list"){
			players = res['players'].map(function(e, i){return e['name']+"("+e['rate']+")"}).join(" VS ");
			$('#players_list').text(players);
			print_console("对战双方: "+players);
		}
