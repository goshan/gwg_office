...
function set_players(players){
	_.each(players, function(e, i){
		$('#players_status .pos'+(i+1)).addClass('player'+e['id']).children('.player_name').text(e['name']+"("+e['rate']+")");
		if (e['self']){
...
function set_heroes(heroes){
	_.each(heroes, function(e, i){
		var hero_status = template_to_html('hero_status_template', e);
		$('#players_status .player'+current_player_id).append(hero_status);
	});
	hero_pos_init();
}
...
function on_hero_status_select(){
	$(".hero_status").live("click", function(e){
		if (current_status == "deploy"){
		}
		else if (current_status == "in_turn" || current_status == "out_of_turn"){
		}
	});
}
